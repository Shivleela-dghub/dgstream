...
export default function Capabilities({ openCap, setOpenCap, hoverCap, setHoverCap }) {
  return (
    <div className="py-10 md:py-14 lg:py-16" id="capabilities">
      <div className="max-w-[1140px] mx-auto px-6 md:px-10 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 lg:gap-16 lg:items-end mb-10 md:mb-12 lg:mb-14">
          ...
          >
            Service areas, each with a full deliverable menu. Most projects
            combine two or three — the packages that perform best are the
            ones where creative and strategy share a brief.
          </p>
        </div>
          ...
          {CAPABILITIES.map((cap, idx) => {
            ...
            return (
              <div key={cap.title} className="border-b" style={{ borderColor: COLORS.border }}>
                <div
                  className="flex items-center justify-between py-5 md:py-6 cursor-pointer gap-3 md:gap-4"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className="text-[0.55rem] md:text-[0.6rem] tracking-[0.1em] md:tracking-[0.12em] uppercase mb-[0.2rem]"
                      style={{ ...mono, color: COLORS.muted }}
                    >
                      {cap.area}
                    </div>
                    <div className="text-[0.95rem] md:text-[1.1rem] font-bold" style={clash}>
                      {cap.title}
                    </div>

                    {/* Tags: show on hover (desktop) OR when open on mobile isn't needed since accordion already opens */}
                    {hoverCap === idx && !isOpen && (
                  ...
                  <div
                    className="w-6 h-6 md:w-7 md:h-7 flex items-center justify-center text-[1rem] md:text-[1.1rem] font-light leading-none flex-shrink-0"
                  ...
                  <div className="pb-6 md:pb-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-5 md:mb-6">
                      {cap.services.map((s) => (
                        ...
                        >
                          <h4 className="text-[0.84rem] md:text-[0.88rem] font-bold mb-1" style={clash}>
                            {s.h}
                          </h4>
                          <p
                            className="text-[0.76rem] md:text-[0.78rem] leading-[1.55] md:leading-[1.6]"
                            style={{ color: COLORS.muted }}
                          >
                            {s.p}
                          </p>
                    ...
                    <div
                      className="text-[0.8rem] md:text-[0.82rem] font-light leading-[1.65] md:leading-[1.7] px-4 md:px-5 py-3 md:py-4 border-l-[3px]"
