...
export default function Process() {
  ...
  return () => clearInterval(interval);
    ...
    return (
        <div className="py-20" style={{ background: COLORS.black, color: COLORS.white }}>
        <div className="max-w-[1140px] mx-auto px-6 md:px-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-end mb-14">
            <div>
              ...
              >
                <span className="block w-[3px] h-[15px]" style={{ background: COLORS.lime }} />
                How a Project Runs
              </div>
              <h2 className="font-extrabold leading-[1.08]" style={{ ...clash, fontSize: "clamp(1.8rem,2.8vw,2.6rem)", letterSpacing: "-0.02em", color: COLORS.white }}>
                From brief to live in four weeks.
              </h2>
            </div>
            <p className="text-[0.92rem] font-light leading-[1.8]" style={{ color: "rgba(255,255,255,0.45)" }}>
...
<div className="hidden lg:flex justify-between items-start gap-6">
    {PROCESS_STEPS.map((step, index) => (
 <div  key={step.title}
  className={`w-[230px] min-h-[500px] flex flex-col border transition-all duration-700 ${
    activeStep === index
      ? "border-lime-400 bg-[#121212] p-6"
...
>
  ...
  style={{
    ...clash,
    color: activeStep === index ? COLORS.white :"rgba(255,255,255,.45)",
  }}
>
  {step.title}
</h3>
 ...
 <div
  className={`h-[120px] transition-opacity duration-500 ${
    activeStep === index
      ? "opacity-100"
      : "opacity-0"
  }`}
