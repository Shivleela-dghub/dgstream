...
export default function NewCaseStudy() {
  ...
  // --- generic helpers for nested state ---
  const setField = (key, value) => setForm(f => ({ ...f, [key]: value }));
  const setNested = (group, key, value) =>
    setForm(f => ({ ...f, [group]: { ...f[group], [key]: value } }));

  // --- array field helpers (about, challenge.points, solution.points) ---
  const addItem = (path) => {
    setForm(f => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // strip empty strings out of array fields before saving
      const payload = {
        ...form,
      ...
      await apiServerClient.post('/casestudies', payload);
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className={section}>
            ...
            <div>
              ...
              <input required className={inp} placeholder="Dirty Dogs"
            ...
            <div>
              ...
              <input className={inp} placeholder="Bengaluru, India"
                value={form.location} onChange={e => setField('location', e.target.value)} />
            </div>
            <div>
              <label className={label}>Tagline</label>
              <textarea className={inp} rows={2} placeholder="Premium corndogs, unforgettable flavors..."
    ...
    <option value="published">Published (live on site)</option>
          ...
          <div className={section}>
            ...
            <div>
              ...
              <input className={inp} placeholder="How do you build a food brand in the digital age?"
                value={form.challenge.heading} onChange={e => setNested('challenge', 'heading', e.target.value)} />
            </div>
            <div>
              <label className={label}>Intro</label>
              <textarea className={inp} rows={3} placeholder="Growth and expansion are essential..."
                value={form.challenge.intro} onChange={e => setNested('challenge', 'intro', e.target.value)} />
            </div>
            <div>
              <label className={label}>Points</label>
              <Repeater path="challenge" values={form.challenge.points} placeholder="Difficulty building a loyal online community." />
            </div>
          </div>
          ...
          <div className={section}>
            ...
            <div>
              <label className={label}>Heading</label>
              <input className={inp} placeholder="DG Stream changes the Digital Presence of..."
                value={form.solution.heading} onChange={e => setNested('solution', 'heading', e.target.value)} />
            </div>
            <div>
              <label className={label}>Intro</label>
              <textarea className={inp} rows={3} placeholder="DG Stream brought about a change..."
                value={form.solution.intro} onChange={e => setNested('solution', 'intro', e.target.value)} />
            </div>
            <div>
              <label className={label}>Points</label>
              <Repeater path="solution" values={form.solution.points} placeholder="Social media became a key driver of engagement." />
            </div>
          </div>
